    ...
    <>
      ...
      <div className="w-full h-fi flex flex-col md:p-4 items-center justify-center sm:flex-row sm:justify-between md:border-x-[2.5em] border-black">
        <div className="w-full h-fit md:w-1/2">
          ...
          <div className="w-16 h-2 my-8 mx-4 bg-slate-900"></div>
          ...
          <p className="mx-4 my-2 text-lg">
            Falling Walls is a <strong>Science Summit</strong> showcases
            <strong> breakthroughs</strong> , fosters
            <strong> science-public dialogue,</strong> and connects
            <strong> global leaders </strong>
            to address pressing <strong> challenges.</strong>
          </p>
          <ul className="border-r-8 border-black w-[96%]">
            {fallingWallsObjectives.map((data) => (
              <li
                key={data.id}
                className="list-disc  mx-8 my-8 transition-all duration-200 cursor-pointer hover:text-slate-600 border-slate-500 "
              >
                <h2 className="font-bold text-lg w-[90%] -tracking-wide">
                  {data.description}
                </h2>
              </li>
            ))}
          ...
          <div className="my-8 px-2">
            <div className="flex items-center justify-evenly">
              ...
              <img
                src={settings}
                alt=""
                className="w-[4em] pointer-events-none"
              />
            </div>
            <ul className="mx-8 mt-4">
              {fallingWallsApproach.map((approach) => (
                <li
                  key={approach.id}
