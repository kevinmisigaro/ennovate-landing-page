...
const NavBar = ({ height, showContactBtn,color }) => {
  ...
  return (
    <header
      className={`h-${height} w-full ${bgColor} left-0 right-0 py-2 md:py-6 mt-4`}
      // style={{color?}}
    >
        ...
        <ul className="hidden md:flex md:justify-center md:items-center gap-4 md:gap-16 md:text-gray-950 ">
          {navLinks.map((navlink) => (
            <li key={navlink.id}>
              <NavLink
                to={navlink.linkTo}
                className="no-underline font-poppins hover:no-underline hover:text-yellow-500 font-semibold leading-normal sm:text-lg lg:text-xl xl:text-2xl text-white md:text-white md:p-2 relative nav-link" // Added nav-link class
              >
                {navlink.name}
      ...
      {navState && (
        <motion.div className="absolute w-[90%] mx-[5%] sm:ml-2 h-fit top-16 rounded-xl bottom-0 flex flex-col items-center justify-center z-50 bg-blue-50"
        // animate={controls}
        // initial={{ scale: 1 }}
        // transition={{
        //   duration: 0.5,
