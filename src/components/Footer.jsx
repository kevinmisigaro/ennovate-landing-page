...
import { footerLinks, footerQuickLinks, socialLinks } from "../utils";
...
function News() {
  return (
    <div className=" botom-0 left-4 w-56 text-right absolute">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex w-full justify-center rounded-md bg-black/20 px-4 py-2 font-normal text-lg text-white hover:bg-black/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75">
            Ennovate News
            <ChevronDownIcon
...
function Footer() {
  return (
    <footer className="h-full py-8 bg-current relative">
      <div className="container mx-auto flex flex-wrap">
        {/* Quick Links Section */}
        <div className="w-full h-fit md:w-1/3 px-4 mb-4 md:mb-0">
          <h2 className="text-[#feba00] text-xl font-semibold mb-4">
            Quick Links
          </h2>
          <ul className="text-white">
            {footerQuickLinks.map((quickLink) => (
              <li
                className="mb-2 my-4  text-white visited:text-white"
                key={quickLink.id}
              >
                <Link
                  to={quickLink.route}
        ...
        <div className="w-full md:w-1/3 px-4 mb-4 md:mb-0">
          ...
          <ul className="flex flex-col items-start">
            {footerLinks.map((link) => (
              <li
                className="mr-4 flex-row flex items-center justify-between my-2"
                key={link.id}
              >
                <img
                  className="text-white hover:text-gray-300 w-[1.2em] h-[1.2em]"
                  src={link.icon}
                />

                <h6 className="text-white ml-2">{link.text}</h6>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="w-full md:w-1/3 px-4 mb-4 md:mb-0">
          <h2 className="text-[#feba00] mt-8 md:mt-0 text-2xl md:text-5xl font-bold mb-4 xl:ml-12">
            Let's Get Social
          </h2>
          <ul className="flex items-center justify-start md:mt-12 md:ml-12">
            {socialLinks.map((socialLink) => (
              <li className="mx-4" key={socialLink.id}>
                <a
                  href={socialLink.url}
                  target="_blank"
                  className="text-white hover:text-gray-300"
                >
                  <img src={socialLink.icon} alt="" className="size-8" />
                </a>
              </li>
            ))}
          </ul>
